...
error_handler_t &error_handler_storage() {
    static error_handler_t handler = nullptr;
    return handler;
}

error_handler_t current_error_handler() {
    return error_handler_storage();
}

} // namespace detail

error_handler_t set_error_handler(error_handler_t handler) {
    error_handler_t previous = detail::current_error_handler();
    detail::error_handler_storage() = handler;
    return previous;
}

namespace {

template <typename T> void swap_impl(int n, T *x, int incx, T *y, int incy) {
...
template <typename T> void copy_impl(int n, const T *x, int incx, T *y, int incy) {
    if (n <= 0 || incx == 0 || incy == 0) {
        return;
    }
...
template <typename T> void axpy_impl(int n, T alpha, const T *x, int incx, T *y, int incy) {
    if (n <= 0 || incx == 0 || incy == 0) {
        return;
    }
...
template <typename T> void scal_impl(int n, T alpha, T *x, int incx) {
    if (n <= 0 || incx == 0) {
        return;
    }
...
template <typename T> T dot_impl(int n, const T *x, int incx, const T *y, int incy) {
    if (n <= 0 || incx == 0 || incy == 0) {
...
template <typename T>
std::complex<T> dotu_impl(int n, const std::complex<T> *x, int incx, const std::complex<T> *y,
                          int incy) {
    if (n <= 0 || incx == 0 || incy == 0) {
...
template <typename T>
std::complex<T> dotc_impl(int n, const std::complex<T> *x, int incx, const std::complex<T> *y,
                          int incy) {
    if (n <= 0 || incx == 0 || incy == 0) {
        return std::complex<T>(0, 0);
    }
...
template <typename T> T nrm2_impl(int n, const T *x, int incx) {
    if (n <= 0 || incx == 0) {
        return static_cast<T>(0);
    }
...
template <typename T> T nrm2_complex_impl(int n, const std::complex<T> *x, int incx) {
    if (n <= 0 || incx == 0) {
        return static_cast<T>(0);
    }
...
template <typename T> T abs1(const std::complex<T> &v) {
    return std::abs(v.real()) + std::abs(v.imag());
}

template <typename T> T asum_impl(int n, const T *x, int incx) {
    if (n <= 0 || incx == 0) {
        return static_cast<T>(0);
    }
...
template <typename T> T asum_complex_impl(int n, const std::complex<T> *x, int incx) {
    if (n <= 0 || incx == 0) {
        return static_cast<T>(0);
    }
...
template <typename T> int iamax_impl(int n, const T *x, int incx) {
    if (n <= 0 || incx <= 0) {
        return 0;
    }
...
template <typename T> int iamax_complex_impl(int n, const std::complex<T> *x, int incx) {
    if (n <= 0 || incx <= 0) {
        return 0;
    }
...
template <typename T> void rot_impl(int n, T *x, int incx, T *y, int incy, T c, T s) {
    if (n <= 0 || incx == 0 || incy == 0) {
        return;
    }
...
template <typename T>
void rot_complex_real_impl(int n, std::complex<T> *x, int incx, std::complex<T> *y, int incy, T c,
                           T s) {
    if (n <= 0 || incx == 0 || incy == 0) {
        return;
    }
...
template <typename T> void rotm_impl(int n, T *x, int incx, T *y, int incy, const T *p) {
    if (n <= 0 || incx == 0 || incy == 0) {
        return;
    }
